...
-- Create policies idempotently (avoid PL/pgSQL compile-time parsing by running plain SQL)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Owners manage chama loans' AND tablename = 'chama_loans'
  ) THEN
    CREATE POLICY "Owners manage chama loans" ON public.chama_loans FOR ALL USING (
      EXISTS (
        SELECT 1 FROM public.groups g
        WHERE g.id = public.chama_loans.group_id
          AND (g.owner_id = auth.uid() OR (g.created_by IS NOT NULL AND g.created_by = auth.uid()))
      )
    );
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Owners manage rotations' AND tablename = 'merry_go_round'
  ) THEN
    CREATE POLICY "Owners manage rotations" ON public.merry_go_round FOR ALL USING (
      EXISTS (
        SELECT 1 FROM public.groups g
        WHERE g.id = public.merry_go_round.group_id
          AND (g.owner_id = auth.uid() OR (g.created_by IS NOT NULL AND g.created_by = auth.uid()))
      )
    );
  END IF;
